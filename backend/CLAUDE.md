...
- **Two JSON configs, keep both in sync.** SignalR serializes separately from HTTP. The camelCase `JsonStringEnumConverter` is set on **both** `AddSignalR().AddJsonProtocol(...)` and `ConfigureHttpJsonOptions(...)` in `Program.cs`. Add a new enum → wire it into both, or it serializes as an int over SignalR.
