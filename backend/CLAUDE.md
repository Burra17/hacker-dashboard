...
- **Domain** — entities, value objects, enums, domain logic. Pure C#, zero external packages. The `DashboardEvent<T>` envelope + `DashboardEventType` live in `Domain/Streaming/` (mirroring `contracts/`) so both the hub and handlers share them. **Mirrored `contracts/` types split by reach:** a shared cross-layer streaming primitive used by API + Application + Infrastructure (e.g. `DashboardEvent<T>`) lives in `Domain/Streaming/`; feature-specific request/response DTOs touched only by Application + API (e.g. `TerminalCommand`/`CommandResult`) live in `Features/{Area}/Common/Dtos/`. Keep Domain free of serialization attributes — enums go over the wire as camelCase strings via a `JsonStringEnumConverter(JsonNamingPolicy.CamelCase)` configured at the API JSON options, not on the type.
